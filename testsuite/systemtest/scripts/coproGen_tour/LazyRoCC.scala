...
class BlackBoxExampleModuleImp(outer: BlackBoxExample, blackBoxFile: String)(implicit p: Parameters)
    ...
    Module(
      new BlackBox( Map( "xLen" -> IntParam(xLen)
        ...
        val io = IO( new Bundle {
                      ...
                      val cmd = chiselTypeOf(roccIo.cmd)
                      val resp = chiselTypeOf(roccIo.resp)
                      val busy = chiselTypeOf(roccIo.busy)
                    })
        override def desiredName: String = blackBoxFile + "_coprocessor"
        addResource(s"/vsrc/coprocessor_$blackBoxFile.sv")
        addResource(s"/vsrc/fu_$blackBoxFile.sv")
      }
    )
  }
  ...
  blackbox.io.cmd <> io.cmd
  io.resp <> blackbox.io.resp
