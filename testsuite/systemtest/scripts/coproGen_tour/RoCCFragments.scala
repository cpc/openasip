...
class WithBlackboxRoCC(op: OpcodeSet = OpcodeSet.custom0) extends Config((site, here, up) => {
  case BuildRoCC => up(BuildRoCC) ++ Seq((p: Parameters) => {
    val coproadder = LazyModule(new BlackBoxExample(OpcodeSet.custom0, "custom")(p))
    coproadder
  })
})
