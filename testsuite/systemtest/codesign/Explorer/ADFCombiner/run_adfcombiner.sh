...
CONNECTING_BUSES="$(grep '<bus name.*>' 3.adf |grep "connect"| wc -l)"
...
CONNECTING_SOCKET_R="$(grep '<socket name.*>' 3.adf |grep "connect_r"| wc -l)"
CONNECTING_SOCKET_W="$(grep '<socket name.*>' 3.adf |grep "connect_w"| wc -l)"
