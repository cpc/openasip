...
ogg_int64_t ogg_page_granulepos(ogg_page *og){
  ...
  ogg_int64_t granulepos=ulong2longlong(page[13]&(0xff));
  granulepos=longlongorulong(longlongshiftleft(granulepos,8),page[12]&0xff);
  granulepos=longlongorulong(longlongshiftleft(granulepos,8),page[11]&0xff);
  granulepos=longlongorulong(longlongshiftleft(granulepos,8),page[10]&0xff);
  granulepos=longlongorulong(longlongshiftleft(granulepos,8),page[9]&0xff);
  granulepos=longlongorulong(longlongshiftleft(granulepos,8),page[8]&0xff);
  granulepos=longlongorulong(longlongshiftleft(granulepos,8),page[7]&0xff);
  granulepos=longlongorulong(longlongshiftleft(granulepos,8),page[6]&0xff);
  return(granulepos);
}
...
int ogg_stream_pagein(ogg_stream_state *os, ogg_page *og){
...

  {
    ...
    while(segptr<segments){
      ...
      os->granule_vals[os->lacing_fill]=long2longlong(-1);
...
int ogg_stream_reset(ogg_stream_state *os){
  ...
  os->b_o_s=0;
  os->pageno=-1;
  os->packetno=ulong2longlong(0);
  os->granulepos=ulong2longlong(0);
...
static int _packetout(ogg_stream_state *os,ogg_packet *op,int adv){
  ...
  if(os->lacing_vals[ptr]&0x400){
    ...
    os->lacing_returned++;
    os->packetno=longlongplusulong(os->packetno, 1);
    return(-1);
  }
