...
TrackerGen::createPorts() {
    ...
    for (auto op : operations_) {
        tracker_
            << InPort("search_hartid_" + op + "_i", HWidth_, WireType::Vector)
...
TrackerGen::createOperation() {
    ...
    CommitIFsave.append(
        If(LHSSignal("(commit_kill_i)"),
           Assign(
               "kill_mem_d[commit_hartid_i][commit_id_i]",
               BinaryLiteral("'1"))));
    CommitIFsave.append(
        If(BitwiseNot(LHSSignal("commit_kill_i")),
           Assign(
               "commit_mem_d[commit_hartid_i][commit_id_i]",
               BinaryLiteral("'1"))));
    ...
    commitassign_1.append(
        Assign("  commit_mem_d[commit_hartid_i][i]", BinaryLiteral("'1")));
    If commitif1_inner(
        LHSSignal(
            "((i > issue_mem_r[commit_hartid_i]) & (i <= commit_id_i))"),
        commitassign_1);
    commitif1_innerblk.append(commitif1_inner);
    forLoop(
        commitfor_1, "integer i = 0; i <= IdBits; i++", commitif1_innerblk);
    If commitif_1(
        LHSSignal("(commit_id_i > issue_mem_r[commit_hartid_i])"),
        commitfor_1);

    CodeBlock commitfor_2;
    CodeBlock commitassign_2;
    CodeBlock commitassign2_blk;
    commitassign_2.append(
        Assign("  commit_mem_d[commit_hartid_i][j]", BinaryLiteral("'1")));
    If commitassign2_inner(LHSSignal("(j <= commit_id_i)"), commitassign_2);
    commitassign2_blk.append(commitassign2_inner);
    forLoop(
        commitfor_2, "integer j = 0; j <= IdBits ; j++", commitassign2_blk);
    commitif_1.elseClause(commitfor_2);

    If commitValidIf_1(
        LHSSignal("(commit_valid_i & (~ commit_kill_i))"), commitif_1);
    ...
    killassign_1.append(
        Assign("  kill_mem_d[commit_hartid_i][k]", BinaryLiteral("'1")));
    If killassign1_inner(LHSSignal("(k > commit_id_i)"), killassign_1);
    killassign1_innerblk.append(killassign1_inner);
    forLoop(
        killfor_1, "integer k = 0; k <=tracker_maxval; k++",
        killassign1_innerblk);

    killassign_2.append(
        Assign("  kill_mem_d[commit_hartid_i][l]", BinaryLiteral("'1")));
    If killassign2_inner(
        LHSSignal("(l <= issue_mem_r[commit_hartid_i])"), killassign_2);
    killassign2_innerblk.append(killassign2_inner);
    forLoop(
        killfor_1, "integer l = 0; l <= IdBits; l++", killassign2_innerblk);
    If killif_1(
        LHSSignal("(commit_id_i > issue_mem_r[commit_hartid_i] )"),
        killfor_1);
    ...
    killassign_3.append(
        Assign("  kill_mem_d[commit_hartid_i][p]", BinaryLiteral("'1")));
    If killassign3_inner(
        LHSSignal("((p >commit_id_i) & (p <= issue_mem_r[commit_hartid_i]))"),
        killassign_3);
    killassign3_innerblk.append(killassign3_inner);
    forLoop(
        killfor_3, "integer p = 0; p <= IdBits; p++", killassign3_innerblk);
    killif_1.elseClause(killfor_3);

    If killValidIf_2(
        LHSSignal("(commit_valid_i & (commit_kill_i))"), killif_1);

    // Removing the ID of the output data
    CodeBlock Outputvalid;
    Outputvalid.append(Assign(
        "commit_mem_d[output_hartid_i][output_id_i]", BinaryLiteral("'0")));
    Outputvalid.append(Assign(
        "kill_mem_d[output_hartid_i][output_id_i]", BinaryLiteral("'0")));
    ...
    for (auto op : operations_) {
        ...
        If Checkcommitmem(
            LHSSignal(
                "(commit_mem_r[search_hartid_" + op + "_i][search_id_" + op +
                "_i])"),
            Assign("out_committed_" + op + "_o", BinaryLiteral("'1")));
        Memtraverse.append(Checkcommitmem);
        If Checkkillmem(
            LHSSignal(
                "(kill_mem_r[search_hartid_" + op + "_i][search_id_" + op +
                "_i])"),
        ...
        DefaultVals.append(
            Assign("out_committed_" + op + "_o", BinaryLiteral("'0")));
        DefaultVals.append(
            Assign("out_killed_" + op + "_o", BinaryLiteral("'0")));
    ...
    TheLogic << DefaultVals << commitValidIf_1 << killValidIf_2 << OutputIf
             << SearchBlk;
