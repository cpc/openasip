...
class FUMergeMinimizer : public DesignSpaceExplorerPlugin {
    PLUGIN_DESCRIPTION("Explorer plugin that greedily merges FUs which are "
                       "rarely used simultaneously.");

    FUMergeMinimizer()
        : DesignSpaceExplorerPlugin(), numLSUs_(1), dontMerge_("") {
        addParameter(numLSUsPN_, UINT, false, Conversion::toString(numLSUs_),
    ...
    virtual void benchmark(TTAMachine::Machine* mach,
        ...
        for (std::set<RowID>::const_iterator id = applicationIDs.begin();
            ...
            simulate(*scheduledProgram, *mach, testApplication, 0,
    ...
    explore(const RowID& configurationID, const unsigned int&) {
        ...
        while (true) {
            ...
            if (fuCount == (1 + numLSUs_ + nonMergeableUnits))
                break;
            ...
            for (int j = 0; j < fuCount - 1; j++) {
                if (!canMerge(fuNavi.item(j)->name()))
                    continue;

                for (int k = j + 1; k < fuCount; k++) {
                    if (!canMerge(fuNavi.item(k)->name()))
                        continue;
            ...
            verboseLog("Created a new configuration " +
...
private:
     ...
     */
    bool hasNumLSUsLeft(Machine* mach, int amount) {
     ...
     */
    bool canMerge(std::string fuName) {
        ...
        for (int i = 0; i < fuB->operationCount(); i++) {
            ...
            for (int j = 1; j <= osalOp.operandCount(); j++) {
                ...
                if (foundPort) {
                    ...
                    portA = createFUPort(fuA, operandWidth, operand.isInput(),
                        portB->isTriggering());
     ...
     * @param mach Given TTAMachine.
     */
    void removeUnusedBuses(TTAMachine::Machine* mach) {
     ...
     * @ param mach Given TTAMachine.
     */
    void removeUnconnectedSockets(TTAMachine::Machine* mach) {
    ...
    FUPort* createFUPort(
        ...
        FUPort* port =
            new FUPort(prefix + Conversion::toString(idx) + postfix, width,
                *fu, false, false);
     ...
     */
    Socket* createSocket(Machine* mach) {
        unsigned int idx = 0;
