...
class BusMergeMinimizer : public DesignSpaceExplorerPlugin {
    PLUGIN_DESCRIPTION("Shrinks machine by merging busses with "
            ...
            */
            ...
            for (int i = 0; i < socketNavi.count(); i++) {
                ...
                for (int j = i + 1; j < socketNavi.count(); j++) {
                    ...
                    if (!dynamic_cast<RFPort*>(socketA->port(0)))
                        continue;
...
protected:
     ...
     */
    void findImmBuses(TTAMachine::Machine* mach, int* immBuses) {
        for (int i = 0; i < IMM_SLOT_COUNT; ++i)
            immBuses[i] = -1;
