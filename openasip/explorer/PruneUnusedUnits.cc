...
class PruneUnusedUnits : public DesignSpaceExplorerPlugin {
    PLUGIN_DESCRIPTION("Compiles the program and prunes FUs which are not "
                       "used.");

    PruneUnusedUnits()
        : DesignSpaceExplorerPlugin(), skeleton_(""), mode_("scalar") {
        addParameter(skeletonPN_, STRING, false, skeleton_,
            "Skeleton IO. List of predefined FUs in starting arch.");
        addParameter(modePN_, STRING, false, mode_,
    ...
    explore(const RowID& configurationID, const unsigned int&) {
        ...
        if (mach->isLittleEndian()) {
            lsOps = {"ld8", "ldu8", "ld16", "ldu16", "ld32", "st8", "st16",
                "st32"};
        } else {
            lsOps = {
                "ldq", "ldqu", "ldh", "ldhu", "ldw", "stq", "sth", "stw"};
        }
        ...
        while (true) {
            ...
            if (maxWidth < mach->busNavigator().item(0)->width()) {
                verboseLog("Setting bus width to: " +
...
private:
    ...
    std::set<TCEString> minimalOpset_ = {
        "add", "and", "eq", "gt", "gtu", "ior", "sub", "xor", "shr", "shru"};
     ...
     */
    bool isSkeletonFU(TCEString fuName) {
     ...
     */
    void duplicateFUs(Machine* mach, int numCopies) {
     ...
     * @param mach Machine given.
     */
    void cleanupSockets(Machine* mach) {
