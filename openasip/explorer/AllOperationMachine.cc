    ...
    explore(const RowID& configurationID, const unsigned int&) {
        ...
        if (mach->isLittleEndian()) {
            unwantedMemOps = {"ALDH", "ALDHU", "ALDQ", "ALDQU", "ALDW", "AST",
                "ASTH", "ASTQ", "ASTW", "LDQ", "LDQU", "LDH", "LDHU", "LDW",
                "STQ", "STH", "STW"};
        } else {
            unwantedMemOps = {"LD8", "LDU8", "LD16", "LDU16", "LD32", "ST8",
                "ST16", "ST32"};
        }
        ...
        for (int i = 0; i < fuNavi.count(); i++) {
            ...
            if (skeletonFUs.size() > 0) {
                ...
                if (std::find(skeletonFUs.begin(), skeletonFUs.end(),
                        fuName) != skeletonFUs.end()) {
        ...
        for (int mId = 0; mId < index.moduleCount(); mId++) {
            ...
            if (module.name() == "simd" && mode_ == "scalar")
                continue;
        ...
        for (int width : {32, 64, 128, 256, 512, 1024, 2048, 4096}) {
            // Add register file
            RegisterFile* rf =
                new RegisterFile("RF_" + Conversion::toString(width),
