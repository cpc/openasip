...
function fetch_llvm {
    ...
    then
        git clone --single-branch --depth=1 --branch release/16.x https://github.com/cpc/llvmtce.git $llvm_co_dir\
            || eexit "Git clone $REV_TOFETCH from llvm failed"
    else
        cd $llvm_co_dir;
        git checkout release/16.x ||	eexit "checking out git branch failed"
        git fetch
        git reset --hard origin/release/16.x || eexit "resetting --hard HEAD failed"
        cd ..;
    fi
}
