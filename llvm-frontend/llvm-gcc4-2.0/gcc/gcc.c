...
/* APPLE LOCAL LLVM */
"%{!O4:%{!emit-llvm:%{!S:-o %|.s |\n llvm-as %(asm_options) %|.s %A }}}";
#else
/* APPLE LOCAL LLVM */
"%{!O4:%{!emit-llvm:%{!S:-o %|.s |\n llvm-as %(asm_options) %m.s %A }}}";
#endif
...
"/* APPLE LOCAL pch */"\
  ...
  {"@assembler",
   "%{!M:%{!MM:%{!E:%{!S:llvm-as %(asm_debug) %(asm_options) %i %A }}}}", 0, 1, 0},
  {".S", "@assembler-with-cpp", 0, 1, 0},
  {"@assembler-with-cpp",
#ifdef AS_NEEDS_DASH_FOR_PIPED_INPUT
   "%(trad_capable_cpp) -lang-asm %(cpp_options)\
      ...
      %{!M:%{!MM:%{!E:%{!S:-o %|.s |\n\
       llvm-as %(asm_debug) %(asm_options) %|.s %A }}}}"
#else
   "%(trad_capable_cpp) -lang-asm %(cpp_options)\
      %{E|M|MM:%(cpp_debug_options)}\
      %{!M:%{!MM:%{!E:%{!S:-o %|.s |\n\
       llvm-as %(asm_debug) %(asm_options) %m.s %A }}}}"
#endif
   , 0, 1, 0},
