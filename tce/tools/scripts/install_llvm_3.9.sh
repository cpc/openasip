...
function fetch_llvm {
    ...
    then
        svn -q $REV_TO_FETCH co http://llvm.org/svn/llvm-project/llvm/branches/release_39 $llvm_co_dir \
...
function fetch_clang {
    ...
    if ! test -d clang;
    then
        svn -q co http://llvm.org/svn/llvm-project/cfe/branches/release_39 clang \
...
function apply_patches {
    cd $llvm_co_dir
    try_patch $patch_dir/llvm-3.9-custom-vector-extension.patch 
    try_patch $patch_dir/llvm-3.9-tce-and-tcele.patch
