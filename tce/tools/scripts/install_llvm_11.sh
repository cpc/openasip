...
function fetch_llvm {
    ...
    then
        git clone --branch release/11.x https://github.com/cpc/llvmtce.git $llvm_co_dir\
	    || eexit "Git clone $REV_TOFETCH from llvm failed"
    else
	      cd $llvm_co_dir;
          git checkout release/11.x ||	eexit "checking out git branch failed"
