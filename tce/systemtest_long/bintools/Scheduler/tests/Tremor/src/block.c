...
int vorbis_synthesis_init(vorbis_dsp_state *v,vorbis_info *vi){
  ...
  v->pcm_returned=-1;
  v->granulepos=long2longlong(-1);
  v->sequence=long2longlong(-1);
...
int vorbis_synthesis_blockin(vorbis_dsp_state *v,vorbis_block *vb){
  ...
  if(!longlongequals(longlongplusulong(v->sequence,1), vb->sequence))
	v->granulepos=long2longlong(-1); /* out of sequence;
                                                     lose count */
  ...
  
  {
    ...
    if(longlongequals(v->granulepos,long2longlong(-1)))
      if(longlongequals(vb->granulepos, long2longlong(-1))){
	v->granulepos=ulong2longlong(0);
      }else{
	v->granulepos=vb->granulepos;
      }
    else{
      v->granulepos = longlongplus(v->granulepos, long2longlong(ci->blocksizes[v->lW]/4+ci->blocksizes[v->W]/4));
      if(!longlongequals(vb->granulepos, long2longlong(-1)) && !longlongequals(v->granulepos,vb->granulepos)){

	if(longlonggreater(v->granulepos, vb->granulepos)){
	  long extra=longlong2long(longlongminus(v->granulepos,vb->granulepos));

	  if(vb->eofflag){
	    /* partial last frame.  Strip the extra samples off */
	    v->pcm_current-=extra;
	  }else if(longlongequals(vb->sequence,ulong2longlong(1))){
...
int vorbis_synthesis_read(vorbis_dsp_state *v,int bytes){
  DPRINTF(("vorbis_synthesis_read(?, %d)\n", bytes));
