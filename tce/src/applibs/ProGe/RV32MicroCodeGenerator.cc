...
RV32MicroCodeGenerator::RV32MicroCodeGenerator(const Machine& machine,
const BinaryEncoding& bem) :
    machine_(&machine), bem_(&bem), RF_(NULL),
    rs1Bus_(NULL), rs2Bus_(NULL), rdBus_(NULL),
    rs1RFPort_(NULL), rs2RFPort_(NULL), rdRFPort_(NULL),
    pig_(new ProgramImageGenerator()), NOP_(""),
    bypassInstructionRegister_(false), hasForwarding_(false),
    variableLengthOpLatency_(false), eVariant_(false) {
    ...
    if (RV32_RTL_GEN_VERBOSE) {
        if (hasForwarding_) {
            ...
            std::cout <<
            "Not required connectivity for bypass logic" << std::endl;
        }
    }
}
    ...
    for (int i = 0; i < bem_->instructionFormatCount(); i++) {
        ...
        for (const auto& op : ops) {
            ...
            if (fTemp.name() == "riscv_r_type") {
                ...
                if (MapTools::containsKey(operationNameTable, name)) {
                    const std::string tceOpName = operationNameTable.at(name);
                    bOperations_.insert({operationNameTable.at(name), 
                    encBits});
                } else {
...
void
RV32MicroCodeGenerator::throwTriggeringPortError(const BaseFUPort* port,
const std::string& type) const {
    std::string fuName = port->parentUnit()->name();
    std::string msg = type + " operand port \'" + port->name() + "\' in FU \'"
    + fuName + "\' not mapped as triggering port";
        throw InvalidData(__FILE__, __LINE__, __func__, msg);
}

void
RV32MicroCodeGenerator::throwInputPortError(const BaseFUPort* port,
const std::string& type) const {
    std::string fuName = port->parentUnit()->name();
    std::string msg = type + " operand port \'" + port->name() + "\' in FU \'"
    + fuName + "\' not mapped as input";
        throw InvalidData(__FILE__, __LINE__, __func__, msg);
}

void
RV32MicroCodeGenerator::throwOutputPortError(const BaseFUPort* port,
const std::string& type) const {
    std::string fuName = port->parentUnit()->name();
    std::string msg = type + " operand port \'" + port->name() + "\' in FU \'"
    + fuName + "\' not mapped as output";
        throw InvalidData(__FILE__, __LINE__, __func__, msg);
}
...
RV32MicroCodeGenerator::connectRF() {
    ...
    for (int l = 0; l < 2 && !success; l++) {
        ...
        for(unsigned int i = 0; i < RFOutputPorts.size() && !success; i++) {
            ...
            for(unsigned int j = 0; j <  tmpRFOutputPorts.size()
            && !success; j++) {
                rs2RFPort = tmpRFOutputPorts.at(j);
                for(unsigned int k = 0; k < RFInputPorts.size()
                && !success; k++) {
                    ...
                    success = findConnectedBusses(
                        rs1, rs2, rd, simm, forwarding);
                    if(success) {
                        break;
                    }
                }
    ...
    if (!success) {
        std::string msg = "Could not find enough connectivity in the" 
        "interconnect to construct RISC-V control and decode logic. Make sure"
...
bool
RV32MicroCodeGenerator::findConnectedBusses(Connection& rs1, Connection& rs2,
Connection& rd, Connection& simm, const bool& forwarding) const { 
    ...
    for (unsigned int i = 0; i < busses_.size(); i++) {
        ...
        for (unsigned int j = 0; j < tmpBusses.size(); j++) {
            ...
            for (unsigned int k = 0; k < tmpBusses.size(); k++) {
                ...
                if (!MachineConnectivityCheck::isConnected(rdPorts, *tmpBus)) {
                    continue;
                }
                rd.bus = tmpBus;
                ...
                for (unsigned int h = 0; h < tmpBusses.size(); h++) {
                    ...
                    if (MachineConnectivityCheck::isConnected(
                        simmPorts, *tmpBus)  && tmpBus->immediateWidth() > 0) {
                        simm.bus = tmpBus;
                        return true;
                    }
                }
...
RV32MicroCodeGenerator::constructRInstructions(
    Port* src1, Port* src2) const {
    std::unordered_map<std::string, InstructionBitVector*> retval;
    CodeCompressorPlugin* compressor = &pig_->compressor();
    for(const auto& op : rOperations_) {
        Instruction* instruction = new Instruction();
        HWOperation* operation = mapFunctionUnit(op.first)->operation(
            op.first);
        ...
        InstructionBitVector* bits = compressor->bemInstructionBits(
            *instruction);
        delete instruction;
...
std::unordered_map<std::string, InstructionBitVector*>
RV32MicroCodeGenerator::constructIInstructions(Port* src1, Port* /*src2*/)
const {
    std::unordered_map<std::string, InstructionBitVector*> retval;
    CodeCompressorPlugin* compressor = &pig_->compressor();
    for(const auto& op : iOperations_) {
...
std::unordered_map<std::string, InstructionBitVector*>
RV32MicroCodeGenerator::constructSInstructions(
    Port* src1, Port* src2) const {
...
std::unordered_map<std::string, InstructionBitVector*>
RV32MicroCodeGenerator::constructBInstructions(
    Port* src1, Port* src2) const {
    std::unordered_map<std::string, InstructionBitVector*> retval;
    CodeCompressorPlugin* compressor = &pig_->compressor();
    for (const auto& op : bOperations_) {
...
RV32MicroCodeGenerator::constructUJInstructions() const {
    ...
    for (const auto& op : ujOperations_) {
        Instruction* instruction = new Instruction();
        if (op.first == "move") {
            HWOperation* operation = mapFunctionUnit("add")->operation(
                "add");
            ...
            HWOperation* operation = mapFunctionUnit(
                op.first)->operation(op.first);
...
RV32MicroCodeGenerator::addBitsToMap(
    ...
    const std::map<std::string, std::string> encodings, std::ofstream& stream)
    const {
    assert(instructions.size() == encodings.size());
    for(const auto& op : instructions) {
        stream << "    elsif(fu_opcode(" << encodings.at(op.first).size()
        << " - 1 downto 0) = " << "\"" << encodings.at(op.first) << "\""
        << ") then" << std::endl << "      moves <= \""
        <<op.second->toString() << "\";" << std::endl;
        delete op.second;
    }
}
    ...
    for (const auto& p : sourceOperationMap_) {
        ...
        for (const auto& op : p.second) {
            ...
            if (op == "add") {
                ...
                << ujOperations_.at("move") << "\"" <<  " then\n";

            } else if (
                iOperations_.count(op) && rOperations_.count(op)) {
                int iLen = iOperations_.at(op).size();
                stream << cond << "fu_opcode(" << iLen << "-1 downto 0)" <<
                " = \"" << iOperations_.at(op) << "\"  or "   <<
                "fu_opcode = \"" << rOperations_.at(op)
            ...
            stream << "      target_fu <= to_unsigned(" << id << ", "
                   << len << ");\n";
        }
        id++;
    }
...
void
RV32MicroCodeGenerator::addRs1ForwardingConditions(std::map<std::string, std::string>
...
RV32MicroCodeGenerator::addRs2ForwardingConditions(
    std::map<std::string, std::string> ops, std::unordered_map<std::string,
    InstructionBitVector*>(ProGe::RV32MicroCodeGenerator::*instructionFunc)
    (Port* p1, Port* p2) const, std::ofstream& stream) const {
    bool firstOpcodeCond = true;
    for (const auto& op : ops) {
        std::string opcodeCond = "        elsif ";
        if (firstOpcodeCond) {
...
RV32MicroCodeGenerator::generateMap(const std::string& dstDirectory) {
    ...
    */
    ...
    stream
    << "  fu_opcode_in : in  std_logic_vector(16 downto 0);"      << std::endl
    ...
    << "  signal fu_opcode : std_logic_vector(16 downto 0);"   << std::endl
    ...
    if (hasForwarding_) {
        int len = std::ceil(std::log2(sourceOperationMap_.size()));
        stream << "  signal target_fu : unsigned(" << len
        << "-1 downto 0);" << std::endl;
    ...
    if (hasForwarding_) {
        ...
        addRs1ForwardingConditions(rOperations_,
        &ProGe::RV32MicroCodeGenerator::constructRInstructions, stream);
        addRs1ForwardingConditions(bOperations_,
        &ProGe::RV32MicroCodeGenerator::constructBInstructions, stream);
        addRs1ForwardingConditions(sOperations_,
        &ProGe::RV32MicroCodeGenerator::constructSInstructions, stream);
        addRs1ForwardingConditions(iOperations_,
        &ProGe::RV32MicroCodeGenerator::constructIInstructions, stream);
        stream
        << "      end if;\n"
        << "    if(rs2_hazard = '1') then\n";
        addRs2ForwardingConditions(rOperations_,
        &ProGe::RV32MicroCodeGenerator::constructRInstructions, stream);
        addRs2ForwardingConditions(bOperations_,
        &ProGe::RV32MicroCodeGenerator::constructBInstructions, stream);
        addRs2ForwardingConditions(sOperations_,
        &ProGe::RV32MicroCodeGenerator::constructSInstructions, stream);
        stream
        << "      end if;\n"
        << "    end if;\n";
...
std::string
RV32MicroCodeGenerator::generateOperationLatencyLogic(HDLTemplateInstantiator& 
instantiator) {
    ...
    instantiator.replacePlaceholder("op-lat-width", std::to_string(
    len));
    if (opLatency.size() == 0) {
    ...
    for(const auto& op : opLatency) {
        ...
        if (rOperations_.count(op.first) && iOperations_.count(op.first)) {
            cond += "fu_opcode = \"" + rOperations_.at(op.first) + 
            "\" or " + "fu_opcode(" +
            ...
            + "-1 downto 0) =  \"" + operations.at(op.first) + "\" then\n";
        }
        logic += cond + 
        "     op_latency <= to_unsigned(" + std::to_string(op.second) +
        ",op_lat_width_c);\n";
    }
    logic +=
    "  else\n"
...
RV32MicroCodeGenerator::generateWrapper(
    ...
    instantiator.replacePlaceholder("rs1-bus-width", std::to_string(
        rs1BusWidth_));
    instantiator.replacePlaceholder("rs2-bus-width", std::to_string(
        rs2BusWidth_));
    instantiator.replacePlaceholder("rd-bus-width",  std::to_string(
        rdBusWidth_));
    instantiator.replacePlaceholder("simm-bus-width",std::to_string(
        simmBusWidth_));

    instantiator.replacePlaceholder("rs1-bus-start", std::to_string(
        rs1BusStart_));
    instantiator.replacePlaceholder("rs2-bus-start", std::to_string(
        rs2BusStart_));
    instantiator.replacePlaceholder("rd-bus-start",  std::to_string(
        rdBusStart_));
    instantiator.replacePlaceholder("simm-bus-start",std::to_string(
        simmBusStart_));

    instantiator.replacePlaceholder("rs1-rf-start", std::to_string(
        rs1RFStart_));
    instantiator.replacePlaceholder("rs2-rf-start", std::to_string(
        rs2RFStart_));
    instantiator.replacePlaceholder("rd-rf-start",  std::to_string(
        rdRFStart_));
    instantiator.replacePlaceholder("simm-rf-start",std::to_string(
        simmRFStart_));
    
    if (eVariant_) {
        instantiator.replacePlaceholder("rf-width",std::to_string(
        4));
    } else {
        instantiator.replacePlaceholder("rf-width",std::to_string(
        5));
    ...
    instantiator.replacePlaceholder("nop-instruction", NOP_);
        instantiator.replacePlaceholder("rv32-microcode-simm-assign",
        "simm_out <= simm_r;");

    const std::string DS = FileSystem::DIRECTORY_SEPARATOR;
    const std::string templateDir = Environment::dataDirPath("ProGe");

    instantiator.replacePlaceholder("rv32-microcode-instruction-assign",
    ...
    if (hasForwarding_) {
        ...
        instantiator.replacePlaceholder("data-hazard-stall",
        "and data_hazard = '0'");
        instantiator.replacePlaceholder("data-hazard-assign-conds",
    ...
    instantiator.replacePlaceholder(
    "rv32-microcode-op-latency-process",generateOperationLatencyLogic(
        instantiator));
    ...
    instantiator.replacePlaceholder(
    "rv32-microcode-instruction-pipeline-logic", instructionPipelineStates);

    instantiator.replacePlaceholder(
    "rv32-microcode-fsm-sensitivity-list-signals",
    fsm_sensitivity_list_signals);
    instantiator.replacePlaceholder("rv32-microcode-execute-logic",
    executeLogic);
...
void
RV32MicroCodeGenerator::generateRTL(HDLTemplateInstantiator& instantiator,
