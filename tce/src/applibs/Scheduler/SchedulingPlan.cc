 ...
 * @param verbose Indicates that frontend should print some information about
 ...
 */
void
SchedulingPlan::build(const ObjectState& conf, bool verbose)
    ...
    for (int i = 0; i < conf.childCount(); i++) {
        ...
        for (int j = 0; j < pass->childCount(); j++) {
            ...
            if (fileName.length() >= 3 && fileName.rfind(".py") == fileName.length() - 3) {
              ...
              base =
                  &pluginLoader_->loadModule(pluginName, fileName, pluginOptions, verbose);
 ...
 * from.
 * @param verbose Indicates frontend should print out
 * information about modules loaded
...
SchedulingPlan::loadFromFile(
    ...
    return serializer.readConfiguration(verbose);
}
